...
const useDynamoHistory = (
  ...
  const [isBasketReady, setBasketReady] = useState(basketReady)

  useEffect(() => {
    if (!isBasketReady && Object.keys(basket).length > 0) {
      updateBasket({})
    }
  }, [isBasketReady])
  ...
  useEffect(() => {
    // Assign the pageType of the first object as primary if not explicitly specified
    if (history.length > 0 && !history[0].pageType) {
      console.log('wronngggggg here', history)
      setHistory(prevHistory => {
        const updatedFirstObj = { ...prevHistory[0], pageType: 'primary' }
...
return previousIndex !== false ? history[previousIndex] : false
  ...
  const goBackToPrimary = (removeAfter = false, skipSecondary = true) => {
    ...
    if (skipSecondary) {
      while (previousIndex >= 0 && history[previousIndex].pageType === 'secondary') {
        previousIndex--
      }
    }
  ...
  const goForwardToType = (removeAfter = false, skipSecondary = true, pageType = 'primary') => {
    if (history.length === 0) return

    let nextIndex = currentIndex + 1
    if (skipSecondary) {
      while (nextIndex < history.length && history[nextIndex].pageType !== pageType) {
        nextIndex++
      }
    }
...
return // Duplicate object found, do not insert or replace
    ...
    const objectType = previousObject && previousObject.pageType === 'primary' ? 'secondary' : 'primary'

    const objectWithDefaultType = {
      ...object,
      ...(!object.pageType && { pageType: objectType })
    }
  ...
  const updateCurrent = updatedObject => {
    ...
    // If the current object is the first object and its pageType is secondary,
    // replace the pageType with "primary"
    if (currentIndex === 0 && updatedObject.pageType === 'secondary') {
      updatedObject.pageType = 'primary'
    }
  ...
  const insertBasket = data => {
    updateBasket({ ...basket, ...data })
  }
  ...
  const toggleBasket = () => {
    setBasketReady(previousState => !previousState)
  }

  const makeBasket = (data = {}) => {
    if (!basketReady) {
      setBasketReady(true)
      insertBasket(data)
    }
  ...
  const getCurrentBasket = () => {
    ...
    for (let i = history.length - 1; i >= 0; i--) {
      ...
      basket = { ...basket, ...snapshot }

      // if (history[i].pageType === "primary") {
      //   break;
      // }
    }
